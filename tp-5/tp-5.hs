...
-- LAS JUSTIFICACIONES ESTAN EN EL ARCHIVO ej-5.hs
...
-- e. uncurry Rect swap :: No tipa (Error de tipos) 
-- f. compose uncurry Pote :: Gusto -> (Gusto, Gusto) -> Helado 
-- g. compose Just :: (a -> b) -> a -> Maybe b 
-- h. compose uncurry (Pote Chocolate) :: No tipa (Error de tipos)

-- ej 6
-- uncurry Rect (1.0, 1.0)
-- compose (uncurry Rect) swap (1.0, 1.0)
-- compose uncurry Pote Chocolate (Vainilla, Vainilla)
-- compose Just doble 2 
...
empty :: Set a
-- que describe el conjunto vacío
empty = (S (\x -> False))

singleton :: (Eq a) => a -> Set a
-- que dado un elemento describe un conjunto que contiene a ese único elemento
singleton x = (S (\y -> x == y))

union :: Set a -> Set a -> Set a
-- que dados dos conjuntos, describe al conjunto que resulta de la unión de ambos
union (S f) (S g) = S (\x -> f x || g x)

intersection :: Set a -> Set a -> Set a
-- que dados dos conjuntos, describe al conjunto que resulta de la intersección de ambos.
...
tryCatch (Ok a) f _ = f a
tryCatch (Raise exception) f handler = handler exception
